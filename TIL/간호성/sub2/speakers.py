...
class SpeakerManager:
    ...
    def compute_speaker_embedding(self, wav_file: str):
        ...
        t = torch.from_numpy(sig).to(device)
        print(t.size())
        t = t.unsqueeze(0)
        print(t.size())

        d_vector = self.speaker_encoder.compute_embedding(t)
    ...
    def compute_d_vector_from_clip(self, wav_file: Union[str, list]) -> list:
        ...
        embeddings = []
        for wav in wav_file:
          embeddings.append(self.compute_speaker_embedding(wav))
        
        temp = embeddings[0]
        for i in range(1, len(embeddings)):
          temp = temp + embeddings[i]

        temp = torch.div(temp, len(embeddings))
